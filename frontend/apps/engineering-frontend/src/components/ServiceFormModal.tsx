...
interface ServiceFormModalProps {
  ...
  mode: "create" | "edit";
  initialServiceName?: string; // optional prefill when creating
}
...
const ServiceFormModal: React.FC<ServiceFormModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    deskripsi_id: "",
    deskripsi_text: "",
    team_members: [{ type: "INTERNAL", team_id: "" }],
    fase_proyek_id: "",
    sbu_id: "",
    unit: "Jam",
    default_duration: "",
  ...
  const [teamType, setTeamType] = useState<"INTERNAL" | "FREELANCE">("INTERNAL");
  const [teamOptions, setTeamOptions] = useState<TaxonomyItem[]>([]);
    ...
    if (open) {
      if (mode === "edit" && service) {
        setFormData({
          ...
          deskripsi_text: (service.deskripsi_text as string) || (service.deskripsi as string) || "",
          team_members: service.team_members
            ? service.team_members.map((m: any) => ({ type: m.type || "INTERNAL", team_id: m.id }))
            : [{ type: "INTERNAL", team_id: "" }],
  ...
  }, [open, mode, service, initialServiceName]);
  ...
  const fetchTaxonomyData = async () => {
    try {
      const [sysCategories, svcCategories, descs, teams, fases, sbuList] = await Promise.all([
        ...
        taxonomyService.getTeamRecommendations(undefined, "INTERNAL"),
      ...
      // set initial team options based on default teamType
      setTeamOptions(teams.filter((item) => item.id));
    } catch {
      showError("Gagal memuat data taxonomy");
    }
  };

  // Load team options depending on selected type
  useEffect(() => {
    let mounted = true;
    const load = async () => {
      try {
        if (teamType === "INTERNAL") {
          const emps = await taxonomyService.getEmployees();
          if (mounted) setTeamOptions(emps.filter((i) => i.id));
        } else {
          const freel = await taxonomyService.getTeamRecommendations(undefined, "FREELANCE");
          if (mounted) setTeamOptions(freel.filter((i) => i.id));
        }
      } catch {
        // ignore
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const serviceData = {
        ...
        // free-text description; backend will create a ServiceDescription row and link it
        deskripsi_text: formData.deskripsi_text || undefined,
        rekomendasi_tim_ids: formData.team_members.map((tm) => tm.team_id),
    ...
    <>
      ...
      >
        <DialogTitle
          ...
          sx={{ pb: 1, px: 2, pt: 2, background: "transparent" }}
        >
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <Box display="flex" gap={1} alignItems="center">
                ...
                <Tooltip title="Tambah Fase Proyek">
                  <span>
                    <IconButton
                      onClick={() => setOpenAddFase(true)}
                      disabled={loading}
                      aria-label="Add Fase Proyek"
                    >
                      <AddIcon />
                    </IconButton>
                  </span>
              ...
              <Box display="flex" gap={1} alignItems="center">
                <Box flex={1}>
                  <Autocomplete
                    ...
                    getOptionLabel={(option) => option.name}
                    value={
                      systemCategories.find((s) => s.id === formData.kategori_sistem_id) || null
                    }
                    onChange={(_, newValue) => {
                ...
                <Tooltip title="Tambah Kategori Sistem">
                  <span>
                    <IconButton
                      onClick={() => setOpenAddSysCat(true)}
                      disabled={loading}
                      aria-label="Add Kategori Sistem"
                    >
                      <AddIcon />
                    </IconButton>
                  </span>
              ...
              <Box display="flex" gap={1} alignItems="center">
                <Box flex={1}>
                  <Autocomplete
                    ...
                    getOptionLabel={(option) => option.name}
                    value={
                      serviceCategories.find((s) => s.id === formData.kategori_jasa_id) || null
                    }
                    onChange={(_, newValue) => {
                ...
                <Tooltip title="Tambah Kategori Jasa">
                  <span>
                    <IconButton
                      onClick={() => setOpenAddSvcCat(true)}
                      disabled={loading}
                      aria-label="Add Kategori Jasa"
                    >
                      <AddIcon />
                    </IconButton>
                  </span>
              ...
              <Box display="flex" gap={1} alignItems="center">
                <Box flex={1}>
                  <Autocomplete
                    ...
                    getOptionLabel={(option) => option.name}
                    value={
                      specificTypes.find((s) => s.id === formData.jenis_jasa_spesifik_id) || null
                    }
                    onChange={(_, newValue) => {
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={(e) =>
                  handleInputChange("deskripsi_text", (e.target as HTMLInputElement).value)
                }
                disabled={loading}
                multiline
                minRows={4}
            ...
            <Grid item xs={12}>
              <Box display="flex" gap={1} alignItems="center">
                <Box sx={{ width: "100%" }}>
                  {formData.team_members.map((tm, idx) => (
                    <Box key={idx} display="flex" gap={1} alignItems="center" mb={1}>
                      <FormControl sx={{ minWidth: 120 }}>
                        <InputLabel>Sumber Tim</InputLabel>
                        <Select
                          value={tm.type}
                          label="Sumber Tim"
                          onChange={(e) => {
                            const newType = e.target.value as "INTERNAL" | "FREELANCE";
                            setFormData((prev) => {
                              const arr = [...prev.team_members];
                              arr[idx].type = newType;
                              arr[idx].team_id = "";
                              return { ...prev, team_members: arr };
                            });
                          }}
                          disabled={loading}
                        >
                      ...
                      <Autocomplete
                        ...
                        options={teamOptions}
                        getOptionLabel={(option) => option.name}
                        value={teamOptions.find((t) => t.id === tm.team_id) || null}
                        onChange={(_, newValue) => {
                          setFormData((prev) => {
                            const arr = [...prev.team_members];
                            arr[idx].team_id = newValue?.id || "";
                            return { ...prev, team_members: arr };
                          });
                        }}
                      ...
                      <IconButton
                        onClick={() => {
                          setFormData((prev) => {
                            ...
                            return {
                              ...prev,
                              team_members: arr.length ? arr : [{ type: "INTERNAL", team_id: "" }],
                            };
                          });
                        }}
                        disabled={loading || formData.team_members.length === 1}
                        aria-label="Remove Tim Member"
                      >
                        <RemoveIcon />
                      </IconButton>
                    </Box>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setFormData((prev) => ({
                        ...prev,
                        team_members: [...prev.team_members, { type: "INTERNAL", team_id: "" }],
                      }));
                    }}
                    disabled={loading}
                    variant="outlined"
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                helperText={
                  !isValidNumber(formData.default_duration)
                    ? "Nilai harus berupa angka (contoh: 1 atau 2.5)"
      ...
      <CreateTaxonomyDialog
        ...
        onCreate={async (name) => {
          try {
            const created = await taxonomyService.createTeamRecommendation(name, teamType);

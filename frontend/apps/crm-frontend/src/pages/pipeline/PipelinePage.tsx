...
const PipelinePage: React.FC = () => {
  ...
  const [selectedSalesUser, setSelectedSalesUser] = useState<string>('all');
  ...
  const [submittingNewOpportunity, setSubmittingNewOpportunity] =
    useState(false);
  ...
  const [boards, setBoards] = useState<BoardMeta[]>(() => {
    ...
    if (savedBoards) {
      try {
        return JSON.parse(savedBoards) as BoardMeta[];
      } catch {
        /* ignore */
      }
    }
    ...
    const initial = seed.map(s => {
      const color = pickRandomColor(usedColors);
      usedColors.push(color);
      return { status: s, title: s.replace(/_/g, ' '), description: '', color };
    });
  ...
  const loadSalesUsers = async () => {
    try {
      ...
      if (
        selectedSalesUser !== 'all' &&
        !result.some(u => u.id === selectedSalesUser)
      ) {
  ...
  const loadPipeline = async () => {
    try {
      ...
      const salesUserId =
        selectedSalesUser === 'all' ? undefined : selectedSalesUser;
      ...
      const response: PipelineResponse = await pipelineApi.getPipeline(
        salesUserId,
        statuses
      );
      ...
      const status = (err as { response?: { status?: number } })?.response
        ?.status; // response shape from axios
  ...
  const defaultStatus = boards && boards.length > 0 ? (boards[0].status as ProjectStatus) : ProjectStatus.PROSPECT;
  ...
  const handleCardClick = async (project: Project) => {
    ...
    try {
      ...
      const maybeCustomer: unknown = (
        detail as unknown as { customer?: unknown }
      ).customer;
      const normalizedCustomer =
        maybeCustomer &&
        typeof maybeCustomer === 'object' &&
        (maybeCustomer as any).customer_name
          ? {
              id: (maybeCustomer as any).id,
              name: (maybeCustomer as any).customer_name,
              city: (maybeCustomer as any).city,
            }
          : (maybeCustomer as { id: string; name?: string; city?: string }) ||
            project.customer;
      ...
      const normalized: Project = {
        ...
        status:
          ((detail as Partial<Project>).status as Project['status']) ||
          project.status,
      };
  ...
  const handleEditBoardSave = () => {
    ...
    const next = boards.map(b =>
      b.status === editingStatus
        ? { ...b, title: title || b.title, description: desc }
        : b
    );
    persistBoards(next);
    setAddingStatus(false);
  ...
  return (
    <Box sx={{ px: { xs: 1, sm: 2, md: 3 }, py: { xs: 0.75, sm: 1, md: 1.5 } }}>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Apakah Anda yakin ingin menghapus board {deleteTarget}? Semua kartu
            akan tetap ada tetapi tidak akan tampil di kolom ini.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button

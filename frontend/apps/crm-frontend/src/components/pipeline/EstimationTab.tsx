...
const EstimationTab: React.FC<EstimationTabProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (attachmentFiles.length > 0) {
        ...
        attachmentUrls.push(
          ...attachmentFiles.map(f => `placeholder://${f.name}`)
        );
      }
  ...
  const canCreateEstimation =
    projectStatus === 'PRE_SALES' && estimations.length === 0;
        ...
        <Box textAlign='center' py={4}>
          ...
          {projectStatus === 'PRE_SALES' ? (
            ...
            <Typography
              ...
              mt={2}
            >
              Permintaan estimasi hanya dapat dibuat saat proyek berada di tahap
              Pre-Sales
            </Typography>
          )}
        </Box>
                ...
                <Box sx={{ '& > *': { mb: 1 } }}>
                  <Box display='flex' alignItems='center'>
                    ...
                    <Typography variant='body2'>
                      ...
                      {(est as any).requested_by_user_name || 'System'}
                    </Typography>
                  </Box>

...
const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({
  ...
  const { user } = useAuth();
  const userDisplay = user?.email || user?.id || 'System';
  ...
  const [salesInfo, setSalesInfo] = useState<{
    ...
    project.sales_user
      ? { name: project.sales_user.name, email: project.sales_user.email }
      : null
  );
  ...
  const checklistStorageKey = (projId?: string) =>
    `pipeline_checklist_${projId}`;
  ...
  useEffect(() => {
    if (!open || !project?.id) return setChecklistItems([]);
    try {
    ...
    try {
      if (project?.id)
        localStorage.setItem(
          checklistStorageKey(project.id),
  ...
  const toggleTodo = (it: ChecklistItem) => {
    const next = checklistItems.map(i =>
      i.id === it.id ? { ...i, is_done: !i.is_done } : i
    );
    setChecklistItems(next);
    persistChecklist(next);
  ...
  const updateTodoDate = (it: ChecklistItem, dueDate: string | null) => {
    const next = checklistItems.map(i =>
      i.id === it.id ? { ...i, due_date: dueDate } : i
    );
    setChecklistItems(next);
    persistChecklist(next);
  };
  ...
  const openDateEditor = (it: ChecklistItem) => {
    ...
    setEditingDateValue(
      it.due_date ? new Date(it.due_date).toISOString().split('T')[0] : ''
    );
  };
  ...
  const handleFormChange = (
    field: string,
    value: ProjectForm[keyof ProjectForm]
  ) => {
  ...
  const handleCreateEstimationRequest = async (data: {
    ...
    try {
      ...
      throw new Error(
        extractErrorMessage(err, 'Gagal membuat permintaan estimasi')
      );
    }
  };
  ...
  return (
    ...
    >
            ...
            <Box>
              {editMode ? (
                <TextField
                  ...
                  onChange={e =>
                    handleFormChange('project_name', e.target.value)
                  }
                  sx={{ minWidth: 320 }}
                />
              ) : (
                    ...
                    <Box sx={{ '& > *': { mb: 1 } }}>
                      <Box display='flex' alignItems='center'>
                        ...
                        <Typography variant='body2'>
                          ...
                          {PIPELINE_COLUMNS[project.status as ProjectStatus]
                      ...
                      <Box display='flex' alignItems='center'>
                        ...
                        {editMode ? (
                          <TextField
                            ...
                            onChange={e =>
                              handleFormChange('lead_score', e.target.value)
                            }
                            sx={{ width: 120 }}
                          />
                        ) : (
                          ...
                          <TextField
                            ...
                            value={
                              form.expected_close_date
                                ? new Date(form.expected_close_date)
                                    .toISOString()
                                    .split('T')[0]
                                : ''
                            }
                    ...
                    <Box display='flex' alignItems='center'>
                      ...
                      {editMode ? (
                        <FormControl size='small' sx={{ minWidth: 240 }}>
                          <InputLabel id='sales-user-label'>
                            Sales Person
                          </InputLabel>
                          <Select
                            ...
                            label='Sales Person'
                            value={
                              salesOptions.some(o => o.id === salesUserId)
                                ? salesUserId
                                : ''
                            }
                            onChange={e =>
                              setSalesUserId(e.target.value as string)
                            }
                          >
                            {user?.id && (
                    ...
                    <Grid item xs={12} md={12}>
                      ...
                      >
                        <Box>
                          ...
                          <Typography variant='body2'>
                            <strong>Email:</strong>{' '}
                            {customerDetails?.customer_contacts?.[0]?.email ||
                              'Belum ada'}
                          </Typography>
                          <Typography variant='body2'>
                            <strong>Phone:</strong>{' '}
                            {customerDetails?.customer_contacts?.[0]?.phone ||
                              'Belum ada'}
                          </Typography>
                          <Typography variant='body2'>
                            <strong>Kota:</strong>{' '}
                          ...
                          <Typography variant='body2'>
                            ...
                            {customerDetails?.district || 'Belum ada'}
                          </Typography>
                          <Typography variant='body2'>
                            <strong>Alamat:</strong>{' '}
                            {customerDetails?.alamat || 'Belum ada'}
                          </Typography>
                ...
                <Grid container spacing={2} alignItems='center' sx={{ mb: 1 }}>
                  ...
                  <Grid item xs={12}>
                    <TextField
                      ...
                      value={
                        newTodoDate
                          ? new Date(newTodoDate).toISOString().split('T')[0]
                          : ''
                      }
                      onChange={e =>
                        setNewTodoDate(e.target.value ? e.target.value : null)
                      }
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                <CheckCircleIcon
                                  color={it.is_done ? 'success' : 'action'}
                                />
                              </IconButton>
                            </Tooltip>
                          </Box>
                          ...
                          <Box sx={{ flex: 1, pr: '120px' }}>
                            <TextField
                              ...
                              value={it.title}
                              onChange={e =>
                                updateTodoTitle(it, e.target.value)
                              }
                              sx={{
                                '& .MuiInputBase-input': {
                                  py: 0.75,
            ...
            <Box textAlign='center' py={4}>
              ...
              {project.status === 'PRE_SALES' ? (
                ...
                <Typography
                  ...
                  mt={2}
                >
                  Permintaan estimasi hanya dapat dibuat saat proyek berada di
                  tahap Pre-Sales
                </Typography>
              )}
            </Box>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Apakah Anda yakin ingin menghapus project "{project.project_name}"? Tindakan ini akan menghapus data dari database dan tidak dapat dikembalikan.
          </Typography>
        </DialogContent>
        <DialogActions>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <TextField
              ...
              onChange={e =>
                setEditingDateValue(e.target.value ? e.target.value : '')
              }
      ...
      <EstimationRequestModal
        ...
        customerName={
          customerDetails?.customer_name || project.customer?.name || 'Customer'
        }
    ...
    <Box>
      ...
      <Alert severity='info' sx={{ mt: 2 }}>
        <Typography variant='caption'>
          📁 Dokumen disimpan secara lokal di browser Anda. 
          Data akan hilang jika browser cache dibersihkan atau menggunakan browser/komputer lain.
        </Typography>
      </Alert>
    </Box>
  );

...
class JournalEntriesService {
    ...
    async getAllJournalEntries(filters) {
      ...
      `;
            const params = [];
            if (filters === null || filters === void 0 ? void 0 : filters.account_id) {
            ...
            const entries = await prisma.$queryRawUnsafe(query, ...params);
            return entries;
        }
        catch (error) {
    ...
    async createJournalEntry(data) {
        ...
        `;
            ...
            // Get account details
            const entryWithAccount = await prisma.$queryRawUnsafe(`
        SELECT je.*, ca.account_code, ca.account_name, ca.account_type
        FROM journal_entries je
        LEFT JOIN "ChartOfAccounts" ca ON je.account_id = ca.id
        WHERE je.id = $1::bigint
    ...
    async updateJournalEntry(id, data) {
        try {
            ...
            const existing = await prisma.$queryRawUnsafe('SELECT * FROM journal_entries WHERE id = $1::bigint', id);
    ...
    async deleteJournalEntry(id) {
        try {
            ...
            await prisma.$queryRawUnsafe('DELETE FROM journal_entries WHERE id = $1::bigint', id);
...
exports.default = new JournalEntriesService();
